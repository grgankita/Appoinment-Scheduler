...
const RelatedProfessionals = ({ speciality, profId }) => {
  ...
  useEffect(() => {
    if (professionals.length > 0 && speciality) {
      const professionalsData = professionals.filter(
        (prof) => prof.speciality === speciality && prof._id !== profId
      );
      setRelProf(professionalsData);
    }
  }, [professionals, speciality, profId]);
  ...
  return (
    <div className="flex flex-col items-center gap-4 my-16 text-[#262626]">
      ...
      <div className="w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-3 sm:px-0">
        {/* slice use kiya hai */}
        {relProf.slice(0, 5).map((item, index) => (
          <div
            onClick={() => {
